...
import { buildConnectionSuggestions, ConnectionSuggestion, networkContext, refreshConnectionSuggestions } from '@/data/mockData';
...
export default function NetworkScreen() {
  ...
  const [connections, setConnections] = useState(() => {
    ...
    // warm cache for initial set
    try {
      // avoid blocking — warm cache asynchronously
      import('@/data/mockData').then((mod) => mod.warmConnectionAvatarCache(items));
    } catch (e) {}
    return items;
  });
  ...
  useEffect(() => {
    const unsubscribe = eventBus.on('network:refresh', () => {
      ...
      // warm cache for refreshed items
      try {
        import('@/data/mockData').then((mod) => mod.warmConnectionAvatarCache(newConnections));
      } catch (e) {}
    });
